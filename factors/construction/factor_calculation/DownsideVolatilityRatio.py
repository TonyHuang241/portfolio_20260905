...
class DownsideVolatilityRatio(_BaseHighFreqFactor):
    factor_name = "DownsideVolatilityRatio"
    factor_type = "volatility"
    report_factor_code = "vol_downRatio"
    calculation_logic = "下行波动率占全部分钟收益率均方根波动率的比例。"
    ...
    def calculate(self):
        ...
        def value(group):
            ...
            selected = returns[returns > 0] if "down" == "up" else returns[returns < 0]
